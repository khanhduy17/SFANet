...
class Model(nn.Module):
    def __init__(self):
        ...
        self.load_vgg()
        self.amp = BackEnd()
        self.dmp_mask = BackEnd()
        self.dmp_nomask = BackEnd()

        self.conv_att = BaseConv(32, 1, 1, 1, activation=nn.Sigmoid(), use_bn=True)
        self.conv_mask_out = BaseConv(32, 1, 1, 1, activation=None, use_bn=False)
        self.conv_nomask_out = BaseConv(32, 1, 1, 1, activation=None, use_bn=False)

    def forward(self, input):
        input = self.vgg(input)
        amp_out = self.amp(*input)
        dmp_mask_out = self.dmp_mask(*input)
        dmp_nomask_out = self.dmp_nomask(*input)

        amp_out = self.conv_att(amp_out)
        dmp_mask_out = amp_out * dmp_mask_out
        dmp_mask_out = self.conv_mask_out(dmp_mask_out)
        dmp_nomask_out = amp_out * dmp_nomask_out
        dmp_nomask_out = self.conv_nomask_out(dmp_nomask_out)

        return dmp_mask_out, dmp_nomask_out, amp_out
